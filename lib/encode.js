  ...
  if (type === 'object') {
    ...
    if (value instanceof ArrayBuffer || ArrayBuffer.isView(value)) {
      ...
      if (ArrayBuffer.isView(value)) {
        internalOffset = value.byteOffset;
      }
      ...
      defers.push({ arraybuffer: arraybuffer, internaloffset : internalOffset, length: length, offset: bytes.length });
      return size + length;
    }
...
function encode(value) {
  ...
  for (var i = 0, l = bytes.length; i < l; i++) {
    ...
    if (defer.bin) {
      ...
      var arr = new Uint8Array(defer.arraybuffer, defer.internaloffset);
